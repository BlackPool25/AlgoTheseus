...
CASES = [
    ("hello world", '#include <iostream>\nint main() { std::cout << "hello world"; return 0; }', "hello world", ""),
    ("empty main", "int main() {}", "", ""),
    ("structured bindings", '#include <iostream>\n#include <utility>\nint main() { auto [a,b] = std::make_pair(2,3); std::cout << a+b; return 0; }', "5", ""),
    ("range-for value", '#include <iostream>\nint main() { int a[]={1,2,3}; int sum=0; for (int x:a) { sum+=x; } std::cout << sum; return 0; }', "6", ""),
    ("range-for ref mutation", '#include <iostream>\nint main() { int a[]={1,2,3}; for (int &x:a) { x*=2; } std::cout << a[0] << a[1] << a[2]; return 0; }', "246", ""),
    ("lambda value capture", '#include <iostream>\nint main() { int x=3; auto f=[x](int y) { return x+y; }; x=9; std::cout << f(2); return 0; }', "5", ""),
    ("lambda ref capture", '#include <iostream>\nint main() { int x=3; auto f=[&x]() { x+=2; }; f(); std::cout << x; return 0; }', "5", ""),
    ("lambda sort comparator", '#include <iostream>\n#include <algorithm>\nint main() { int a[]={2,1,3}; std::sort(a,a+3,[](int x,int y) { return x>y; }); std::cout << a[0] << a[1] << a[2]; return 0; }', "321", ""),
    ("function overloading", '#include <iostream>\nint f(int x) { return x+1; }\ndouble f(double x) { return x+0.5; }\nint main() { std::cout << f(2) << " " << f(2.0); return 0; }', "3 2.5", ""),
    ("switch fallthrough", '#include <iostream>\nint main() { int x=1; int sum=0; switch(x) { case 1: sum+=2; [[fallthrough]]; case 2: sum+=3; break; default: sum=9; } std::cout << sum; return 0; }', "5", ""),
    ("do-while", '#include <iostream>\nint main() { int n=0; do { ++n; } while(n<3); std::cout << n; return 0; }', "3", ""),
    ("break continue early-return", '#include <iostream>\nint f() { int sum=0; for(int i=0;i<10;++i) { if(i==1) continue; if(i==4) break; sum+=i; } if(sum==5) return sum; return -1; }\nint main() { std::cout << f(); return 0; }', "5", ""),
    ("globals read written", '#include <iostream>\nint g=2;\nvoid bump() { g+=3; }\nint main() { bump(); std::cout << g; return 0; }', "5", ""),
    ("const ref params", '#include <iostream>\nint f(const int &x,int &y) { y+=x; return y; }\nint main() { const int x=2; int y=3; int z=f(x,y); std::cout << z << " " << y; return 0; }', "5 5", ""),
    ("new delete", '#include <iostream>\nint main() { int *p=new int(7); int x=*p; delete p; int *a=new int[2]{2,3}; int sum=a[0]+a[1]; delete[] a; std::cout << x << " " << sum; return 0; }', "7 5", ""),
    ("long long double char bool edges", '#include <iostream>\n#include <limits>\nint main() { long long lo=std::numeric_limits<long long>::min(); long long hi=std::numeric_limits<long long>::max(); double d=-0.0; double tiny=std::numeric_limits<double>::min(); char c=\'\\0\'; char z=\'\\x7f\'; bool yes=true; bool no=false; std::cout << lo << " " << hi << " " << d << " " << (tiny>0) << " " << int(c) << " " << int(z) << " " << yes << " " << no; return 0; }', "-9223372036854775808 9223372036854775807 -0 1 0 127 1 0", ""),
    ("cin sum raw_stdin", '#include <iostream>\nint main() { int n; std::cin >> n; long long sum=0; for(int i=0;i<n;++i) { int x; std::cin >> x; sum+=x; } std::cout << sum; return 0; }', "15", "5\n1 2 3 4 5\n"),
    ("operator overloading", '#include <iostream>\nstruct Box { int value; Box operator+(const Box &b) const { return Box{value+b.value}; } };\nint main() { Box a{2}; Box b{3}; Box c=a+b; std::cout << c.value; return 0; }', "5", ""),
    ("exceptions try catch", '#include <iostream>\n#include <stdexcept>\nint main() { int x=0; try { throw std::runtime_error("boom"); } catch(const std::exception &e) { x=7; } std::cout << x; return 0; }', "7", ""),
    ("template function known limitation", '#include <iostream>\ntemplate<typename T> T add(T a,T b) { T c=a+b; return c; }\nint main() { int x=add<int>(2,3); std::cout << x; return 0; }', "5", ""),
    ("define line-shifting macro", '#include <iostream>\n#define BUMP(x) do { \\\n ++x; \\\n x+=2; \\\n} while(0)\nint main() {\n int x=2;\n BUMP(x);\n std::cout << x;\n return 0;\n}', "5", ""),
    ("deep recursion 2000", '#include <iostream>\nint recurse(int n) { if(n==0) return 0; return 1+recurse(n-1); }\nint main() { std::cout << recurse(2000); return 0; }', "2000", ""),
    ("infinite loop 5s guard", 'int main() { while(true) {} }', "", ""),
    ("range-for value vector", '#include <iostream>\n#include <vector>\nint main() { std::vector<int> a{1,2,3}; int sum=0; for(int x:a) { sum+=x; } std::cout << sum; return 0; }', "6", ""),
    ("range-for ref mutation vector", '#include <iostream>\n#include <vector>\nint main() { std::vector<int> a{1,2,3}; for(int &x:a) { x*=2; } std::cout << a[0] << a[1] << a[2]; return 0; }', "246", ""),
    ("lambda sort comparator vector", '#include <iostream>\n#include <algorithm>\n#include <vector>\nint main() { std::vector<int> a{2,1,3}; std::sort(a.begin(),a.end(),[](int x,int y) { return x>y; }); std::cout << a[0] << a[1] << a[2]; return 0; }', "321", ""),
]
...
async def resolve_case(index: int) -> Report:
    ...
    report = Report(name=name, outcome="GRACEFUL-FAIL", message="", cache_hit=resolved.cache_hit, warnings=list(resolved.warnings))
...
def run_case(index: int) -> Report:
    env = dict(os.environ, SANDBOX_MODE="subprocess", CACHE_DIR="/tmp/at-sweep-c", PYTHONDONTWRITEBYTECODE="1")
    ...
    with subprocess.Popen(
        ...
        try:
            ...
            return Report(name=CASES[index][0], outcome="HANG", message="Sweep hard watchdog fired after 45s; worker process group SIGKILL", seconds=time.monotonic()-started)
    if process.returncode != 0:
        return Report(name=CASES[index][0], outcome="GRACEFUL-FAIL", message=f"UNHANDLED worker exit={process.returncode}: {stderr}", seconds=time.monotonic()-started)
